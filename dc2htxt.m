function dc2htxt (xmlfile, xstrt, lat, lon)

clearvars -except xmlfile xstrt lat lon; close all
...
[hb, ~] = ratingx(xmlfile, arr);
if isempty(hb), error('No Rating Curve!!'), end
subplot(2,1,2)
if any(isnan(hb(~isnan(arr))))
    error('Extrapolation of Rating Curve may have failed!')
end
hbuf = ft2m*hb(1:qq);
ndate = ndate(1:qq);
flags = flags(1:qq);
