function gh(xmlfile,Toggle,RIV)
try
clearvars -except xmlfile Toggle RIV; close all

ll = length(xmlfile) -4;
if ~exist(fullfile(Toggle.USGSlistdir,['usgs/', xmlfile]), 'file')
...
xstrt = xml2struct(fullfile(Toggle.USGSlistdir,['usgs/', xmlfile]));
rens = xstrt.Children;
tsl = length(rens);
if tsl <=1
    ...
    if isempty(ind)
        ...
        fprintf('Discharge data found; Calling dc2ht.\n')
        dc2ht(xmlfile, xstrt, lat, lon,Toggle,RIV); return
    else
...
%update names for toolbox compliance
